...
def test_batch_progress_api():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"   當前進度: {progress['current_progress']}/{progress['total_items']}"
            )
            print(f"   進度百分比: {progress['percentage']}%")
            print(
                f"   當前批次: {progress['current_batch']}/{progress['total_batches']}"
            )
...
def test_usage_api():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"   月度使用量: {summary['monthly_usage']}/{summary['monthly_limit']}"
            )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if progress["total_items"] == 0:
                ...
                print(
                    f"   ⚠️  有進行中的批次處理: {progress['current_progress']}/{progress['total_items']}"
                )

...
class BatchProcessor:
    ...
    async def process_single_item(self, filename: str, enhance_image: bool = True, save_detailed_csv: bool = False) -> Dict:
        """處理單個圖片"""
        try:
            # 構建檔案路徑
            ...
            receipt_data = await ai_service.process_receipt_text(ocr_result, structured_data)
    ...
    async def process_batch(self, filenames: List[str], enhance_image: bool = True, save_detailed_csv: bool = False) -> List[Dict]:
        """處理一個批次"""
        batch_results = []
        ...
        for i, filename in enumerate(filenames):
            ...
            # 處理單個圖片
            result = await self.process_single_item(filename, enhance_image, save_detailed_csv)
            batch_results.append(result)
            
            # 更新進度
            self.current_progress += 1
            
            logger.info(f"   檔案 {filename} 處理完成: {'成功' if result['success'] else '失敗'}")
    ...
    async def process_large_batch(self, filenames: List[str], enhance_image: bool = True, save_detailed_csv: bool = False) -> Dict:
        """處理大量圖片，包含頻率控制"""
        ...
        # 分批處理
        batches = [filenames[i:i + self.batch_size] for i in range(0, len(filenames), self.batch_size)]
        ...
        for batch_index, batch_filenames in enumerate(batches):
            ...
            logger.info(f"🔄 處理批次 {self.current_batch}/{self.total_batches}，包含 {len(batch_filenames)} 個檔案")
            logger.info(f"   批次檔案: {batch_filenames}")
            
            # 處理當前批次
            batch_results = await self.process_batch(batch_filenames, enhance_image, save_detailed_csv)
            ...
            if batch_index < len(batches) - 1:
                ...
                logger.info(f"批次 {self.current_batch} 完成，等待 {delay_time:.1f} 秒後處理下一批次...")
    ...
    async def process_ocr_only(self, filenames: List[str], enhance_image: bool = True) -> Dict:
        """
        只執行OCR處理，結果暫存
    ...
    async def process_from_cache(self, batch_id: str, save_detailed_csv: bool = False) -> Dict:
        """
        從暫存處理AI分析
        ...
        if not status_data:
            return {
                "success": False,
                "error": f"找不到批次ID: {batch_id}"
            }
        
        cache_files = status_data["status"]["cache_files"]
        logger.info(f"從暫存處理AI分析，批次ID: {batch_id}, 暫存檔案數: {len(cache_files)}")
    ...
    async def merge_with_existing_csv(self, new_receipts: List, existing_csv_path: str = None) -> Dict:
        """
        將新的收據資料合併到現有CSV檔案
        ...
        try:
            ...
            if existing_csv_path and os.path.exists(existing_csv_path):
                existing_receipts = csv_service.load_receipts_from_csv(existing_csv_path)

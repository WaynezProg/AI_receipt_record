...
def show_csv_format_info():
    ...
    print(
        "   欄位: 商店名稱, 日期, 總金額, 小計, 稅額, 稅率, 收據號碼, 付款方式, 識別信心度, 處理時間, 來源圖片"
    )
    print("   用途: 每個收據一行，顯示基本資訊")

    print("\n🔹 商品明細CSV (receipts_details_*.csv):")
    print(
        "   欄位: 收據來源, 商店名稱, 收據日期, 商品名稱（原始）, 商品名稱（日文）, 商品名稱（中文）, 單價, 數量, 小計"
    )
...
def cleanup_test_files():
    ...
    output_dir = "./data/output"
    test_files = [
        f for f in os.listdir(output_dir) if f.startswith("test_consolidated")
    ]

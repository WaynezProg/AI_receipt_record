...
async def test_optimized_batch_processing():
    ...
    standard_result = await batch_processor.process_large_batch(
        test_images, False, True
    )
    ...
    print("\n⚡ 測試優化批量處理...")
    start_time = time.time()
    optimized_result = await optimized_batch_processor.process_large_batch_optimized(
        test_images, True
    )
    ...
    if standard_time > 0:
        ...
        print(
            f"   時間節省: {((standard_time - optimized_time) / standard_time * 100):.1f}%"
        )

    ...
    try:
        ...
        if save_detailed_csv:
            background_tasks.add_task(
                csv_service.save_detailed_csv, 
                receipt_data, 
                f"detailed_{csv_filename}"
            )
...
async def process_from_cache(
    batch_id: str = Form(...),
    save_detailed_csv: bool = Form(True)
):
    """
    從暫存處理AI分析
    """
...
async def get_summary():
    ...
    try:
        # 統計檔案數量
        receipt_files = len([f for f in os.listdir(settings.upload_dir) 
                           if f.lower().endswith(('.jpg', '.jpeg', '.png'))])
        csv_files = len([f for f in os.listdir(settings.output_dir) 
                        if f.endswith('.csv')])
        ...
        csv_files_list = [f for f in os.listdir(settings.output_dir) 

...
class OptimizedBatchProcessor:
            ...
            try:
                ...
                if "RATE_LIMIT_EXCEEDED" in error_msg or "429" in error_msg:
                    if attempt < retries:
                        ...
                        logger.warning(f"Azure API頻率限制，等待 {wait_time} 秒後重試 ({attempt + 1}/{retries})")
                        await asyncio.sleep(wait_time)
                    else:
                        logger.error(f"OCR處理失敗（頻率限制）: {error_msg}")
                        return {"success": False, "error": f"Azure API頻率限制: {error_msg}"}
                else:
                    if attempt < retries:
    ...
    async def _process_batch_parallel(self, filenames: List[str]) -> List[Dict]:
        ...
        async def process_with_semaphore(filename: str) -> Dict:
            async with azure_semaphore:
                ...
                ocr_result = await self._process_ocr_with_retry(image_path)
                if not ocr_result.get('success'):
                    return {"success": False, "filename": filename, "error": ocr_result.get('error')}
    ...
    async def process_large_batch_optimized(self, filenames: List[str], 
                                          save_detailed_csv: bool = True) -> Dict:
        """優化的大批量處理"""
        ...
        logger.info(f"🚀 開始優化批量處理: {len(filenames)} 個檔案，{len(batches)} 個批次")
        ...
        for batch_idx, batch_filenames in enumerate(batches):
            ...
            for result in batch_results:
                ...
                if result.get('success') and result.get('data'):
                    ...
                    logger.error(f"❌ {result.get('filename', 'unknown')} 處理失敗: {result.get('error')}")
        ...
        return {
            ...
            "csv_files": csv_files,
            "avg_time_per_item": round(total_time / len(filenames), 2) if filenames else 0,
            "deleted_successful": len(successful_receipts) if self.auto_delete_successful else 0,

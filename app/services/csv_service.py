...
class CSVService:
    ...
    def save_receipts_to_csv(
        self, receipts: List[ReceiptData], filename: str = None
    ) -> str:
        """
    ...
    def save_consolidated_csv(
        ...
        try:
                    ...
                    try:
                        ...
                        for item_data in receipt.get("items", []):
                            if isinstance(item_data, dict):
                                item = ReceiptItem(
                                    ...
                                    tax_amount=(
                                        float(item_data.get("tax_amount", 0))
                                        if item_data.get("tax_amount")
                                        else None
                                    ),
                                )
                                items.append(item)
                        ...
                        receipt_obj = ReceiptData(
                            ...
                            confidence_score=float(
                                receipt.get("confidence_score", 0.9)
                            ),
            ...
            # 1. 收據摘要CSV
            summary_filename = (
                f"receipts_summary_{timestamp}.csv"
                if not filename
                else f"summary_{filename}"
            )
            summary_path = self.save_receipts_to_csv(safe_receipts, summary_filename)

            # 2. 詳細商品明細CSV
            details_filename = (
                f"receipts_details_{timestamp}.csv"
                if not filename
                else f"details_{filename}"
            )
            ...
            return {"summary_csv": summary_path, "details_csv": details_path}

        except Exception as e:
            logger.error(f"創建整合CSV失敗: {str(e)}")
            raise

    def save_detailed_items_csv(
        self, receipts: List[ReceiptData], filename: str = None
    ) -> str:
        """
    ...
    def load_receipts_from_csv(self, filepath: str) -> List[ReceiptData]:
        ...
        try:
            ...
            with open(filepath, "r", encoding="utf-8") as csvfile:
                ...
                is_details_csv = (
                    "收據日期" in fieldnames and "商品名稱（原始）" in fieldnames
                )
                ...
                if is_summary_csv:
                    ...
                    for row in reader:
                        try:
                            ...
                            receipt_groups[receipt_key]["total_amount"] += (
    ...
    def get_csv_summary(self, filepath: str) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "date_range": {
                    "earliest": (
                        min(receipt.date for receipt in receipts) if receipts else None
                    ),
                    "latest": (
                        max(receipt.date for receipt in receipts) if receipts else None
                    ),
                },
            }

...
def check_uploaded_files():
    ...
    files = [f for f in os.listdir(upload_dir) if f.lower().endswith(('.jpg', '.jpeg', '.png', '.pdf'))]
...
def test_batch_api():
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            print("✅ 批次進度API正常")
            print(f"   當前進度: {data['progress']['current_progress']}/{data['progress']['total_items']}")
            print(f"   當前批次: {data['progress']['current_batch']}/{data['progress']['total_batches']}")
        else:
